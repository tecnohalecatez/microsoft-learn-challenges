...
1. Debe generar un token de firma de acceso compartido que conceda permisos de lectura a un contenedor de blobs.
¿Qué segmento de código debería usar?
    ...
    Este elemento pone a prueba los conocimientos de los candidatos sobre cómo crear e implementar firmas de acceso compartido.
    El segmento de código que incluye Resource = "c" y sasBuilder.SetPermissions(BlobContainerSasPermissions.Read); generará el token de firma de acceso compartido que concede el permiso de lectura a un contenedor de blobs. El segmento de código que incluye Resource = "b" generará un token de firmas de acceso compartido en el nivel de blob. Los segmentos de código para definir los permisos tienen dos partes: el nombre y el valor de la enumeración. La enumeración denominada BlobContainerSasPermissions define los permisos que se aplicarán en el nivel de contenedor de blobs. La enumeración denominada BlobSasPermissions define los permisos que se aplicarán en el nivel de blob. El valor Lectura define los permisos de lectura y el valor Creación define los permisos de creación.


1. Administra una aplicación de Azure App Service Functions denominada app1 y una cuenta de almacenamiento denominada account1.
Tiene los siguientes requisitos:
App1 debería acceder a account1 sin administrar las credenciales.
La entidad de servicio asociada a app1 no se puede eliminar explícitamente.
Será necesario configurar una entidad de seguridad para app1.
¿Qué entidad de seguridad debería usar?
    - Identidad administrada asignada por el sistema

    Este elemento pone a prueba los conocimientos de los candidatos sobre la implementación de identidades administradas, que forma parte de la implementación de soluciones seguras en la nube.
    Las identidades administradas para los recursos de Azure eliminan la necesidad de administrar las credenciales en el código. Solo hay una identidad administrada asignada por cada recurso y está asociada al ciclo de vida del recurso. Una vez habilitada para app1, creará automáticamente una entidad de servicio sin necesidad de administrar las credenciales y no se podrá eliminar explícitamente.
    Una aplicación de Microsoft Entra ID se define por su único objeto de aplicación, que reside en el inquilino de Microsoft Entra ID donde se registró la aplicación, (conocido como inquilino "principal" de la aplicación). App1 no puede usarla para acceder a una cuenta de almacenamiento sin administrar las credenciales. Una identidad administrada asignada por el usuario se puede crear y asignar a una o varias instancias de un servicio de Azure. Una vez habilitada para app1, una identidad administrada asignada por el usuario creará automáticamente una entidad de servicio sin necesidad de administrar, pero tendrá que eliminarse explícitamente. El tipo de entidad de servicio heredado representa una aplicación heredada (la que se creó antes de que se introdujeran o crearan registros de aplicaciones mediante experiencias heredadas). La entidad de servicio heredada no se puede usar para acceder a una cuenta de almacenamiento sin administrar las credenciales.


1. Tiene previsto crear una jerarquía de espacios de nombres de clave en Azure App Configuration.
Debe separar nombres de clave individuales.
¿Qué carácter debería usar?
    - :

    Este elemento pone a prueba los conocimientos de los candidatos sobre la configuración de la jerarquía de espacios de nombres de clave de App Configuration, que forma parte de la implementación de soluciones seguras en la nube.
    El carácter de dos puntos (:) se usa para separar nombres de claves individuales al crear una jerarquía de espacios de nombres en Azure App Configuration. El carácter asterisco (*) es uno de los caracteres reservados de Azure App Configuration, por lo que no se puede usar para separar nombres de claves individuales al crear jerarquías de espacios de nombres en Azure App Configuration. El carácter coma (,) es uno de los caracteres reservados de Azure App Configuration, por lo que no se puede usar para separar nombres de claves individuales al crear jerarquías de espacios de nombres en Azure App Configuration. El carácter barra diagonal inversa (\) es uno de los caracteres reservados de Azure App Configuration, por lo que no se puede usar para separar nombres de claves individuales al crear jerarquías de espacios de nombres en Azure App Configuration.


1. Tiene una instancia de Azure Cache for Redis.
Tiene los siguientes requisitos:
Los nodos de réplica deben hospedarse en diferentes zonas de disponibilidad.
Se debe usar la memoria no volátil.
Tiene que seleccionar un plan de tarifa.
¿Qué plan de tarifa debería usar?
    - Enterprise Flash

    Este elemento pone a prueba los conocimientos de los candidatos sobre los precios de Azure Cache for Redis, que forma parte de la implementación del almacenamiento en caché para las soluciones.
    El nivel Enterprise Flash es capaz de hospedar nodos de réplica en diferentes zonas de disponibilidad y usa memoria no volátil para reducir costes. El nivel Estándar no es capaz de hospedar nodos de réplica en diferentes zonas de disponibilidad. Los niveles Premium y Enterprise son capaces de hospedar nodos de réplica en diferentes zonas de disponibilidad, pero no usan memoria no volátil para reducir costes.


1. Una plataforma de comercio electrónico planea expandir sus servicios globalmente. La plataforma se hospeda en Azure y utiliza varios servicios de Azure e integraciones de terceros.
Debe diseñar y crear una solución de supervisión sólida que se pueda escalar con la expansión y proporcionar información sobre el rendimiento de la plataforma en distintas regiones.
¿Qué tiene que hacer?
    - Implemente varias instancias de Application Insights para cada región y use Azure Monitor para agregar los datos.

    La opción A es correcta porque la implementación de varias instancias de Application Insights para cada región permite la supervisión localizada y el uso de Azure Monitor para agregar los datos proporcionan una vista centralizada del rendimiento global. La opción B es incorrecta porque una sola instancia puede no escalar de forma eficaz para la supervisión global. La opción D es incorrecta porque la instrumentación manual y el almacenamiento en Azure Blob Storage no proporcionan funcionalidades de supervisión y análisis en tiempo real. La opción C es una prueba web incorrecta y las alertas solo forman parte de una solución de supervisión sólida.


1. Planea usar Azure API Management para la administración de API híbridas y multinube.
Debe crear una puerta de enlace autohospedada para producción.
¿Qué etiqueta de imagen de contenedor debería usar?
    - 2.0.1

    Este elemento pone a prueba los conocimientos de los candidatos sobre las puertas de enlace autohospedadas en Azure API Management.
    En producción, la versión debe estar anclada. La única manera de lograrlo es usar una etiqueta que siga la convención {major}.{minor}.{patch}.. La etiqueta v3 dará como resultado que siempre se ejecutará una versión principal con cada nueva característica y revisión. La etiqueta más reciente se usa para evaluar la puerta de enlace autohospedada. La etiqueta V3-preview debería usarse para ejecutar la imagen de contenedor de versión preliminar más reciente.


1. Crea un tema de Azure Service Bus con un tiempo de vida de mensaje predeterminado de 10 minutos.
Debe enviar mensajes a este tema con un período de vida de 15 minutos. La solución no deberá afectar a otras aplicaciones que usen el tema.
¿Qué debe recomendar?
    - Cree un tema con un período de vida predeterminado de 15 minutos. Envíe los mensajes a este tema.

    Esta pregunta pone a prueba los conocimientos de los candidatos sobre la expiración de mensajes de Azure Service Bus.
    Para evitar afectar a las aplicaciones existentes, no se debe cambiar el tiempo de vida del tema existente. Es necesario crear un nuevo tema. El cambio del período de vida predeterminado del tema afectará a otras aplicaciones. Un período de vida de nivel de mensaje no puede ser mayor que el tiempo de vida del tema. Para evitar afectar a las aplicaciones existentes, no se debe cambiar el tiempo de vida de la cola o tema existente.


1. Tiene una aplicación que requiere puesta en cola de mensajes.
Necesita recomendar una solución que cumpla los requisitos siguientes:
detección automática de mensajes duplicados.
capacidad de enviar 2 MB de mensajes.
¿Qué solución de puesta en cola de mensajes debería recomendar?
    - Nivel Premium de Azure Service Bus

    Este elemento pone a prueba los conocimientos de los candidatos sobre Azure Service Bus.
    Service Bus detecta mensajes duplicados. El nivel Premium es necesario para enviar mensajes de más de 256 KB. Aunque Service Bus detecte mensajes duplicados, el nivel Estándar solo admitirá mensajes de hasta 256 KB de tamaño. Las colas de Azure Storage no admiten la detección de mensajes duplicados. Las colas de Azure Storage no admiten la detección de mensajes duplicados.


1. Una empresa de servicios financieros está implementando un sistema para procesar transacciones que se reciben como mensajes. El sistema debe asegurarse de que las transacciones se procesen una sola vez y en el mismo orden en que se reciben para conservar la integridad de los datos.
Debe diseñar una solución de mensajería que garantice el envío y el procesamiento de mensajes según un modelo FIFO (primero en entrar, primero en salir).
¿Qué dos servicios debe usar?
    - Azure Service Bus con detección de duplicados
    - Azure Service Bus con sesiones habilitadas

    Azure Service Bus con sesiones habilitadas permite el procesamiento FIFO mediante sesiones para garantizar que los mensajes correspondientes se controlan en el orden en que se reciben. La detección de duplicados permite garantizar que el sistema no procese la misma transacción más de una vez. Azure Queue Storage no garantiza el procesamiento FIFO, ya que no admite sesiones y el tiempo de espera de visibilidad solo oculta el mensaje temporalmente de otros consumidores. Azure Event Grid es un servicio de enrutamiento de eventos y no ofrece garantías de FIFO ni sesiones de mensajes.


1. Tiene previsto implementar un mecanismo de almacenamiento para administrar el estado en varios consumidores de la fuente de cambios.
Es necesario configurar el procesador de la fuente de cambios en el SDK de .NET para la API de Azure Cosmos DB for NoSQL.
¿Qué componente debe usar?
    - Contenedor de concesión

    Este elemento pone a prueba los conocimientos de los candidatos sobre cómo configurar el procesador de fuente de cambios como parte del desarrollo de soluciones que usan Azure Cosmos DB.
    El componente de contenedor de concesión actúa como un mecanismo de almacenamiento para administrar el estado entre varios consumidores de la fuente de cambios. El componente delegado es código de la aplicación cliente que implementará la lógica de negocios para cada lote de cambios. El instancia de proceso es una aplicación cliente que escucha los cambios de la fuente de cambios. El componente de contenedor supervisado se supervisa para cualquier operación de inserción o actualización. No sirve como mecanismo de almacenamiento para administrar el estado entre varios consumidores de fuente de cambios.


1. Administre la implementación de una cuenta de Azure Cosmos DB.
Deberá definir lógica personalizada mediante el procesador de la fuente de cambios del SDK de .NET para procesar los cambios que lea la fuente de cambios.
Deberá seleccionar el componente de procesador de la fuente de cambios adecuado.
¿Qué componente debe usar?
    - delegado

    Este elemento pone a prueba los conocimientos de los candidatos sobre la implementación de notificaciones de fuente de cambios en Azure Cosmos DB. El procesador de fuente de cambios de Azure Cosmos DB simplifica el proceso de lectura de la fuente de cambios y se puede usar para distribuir el procesamiento de eventos entre varios consumidores de manera eficaz. Hay cuatro componentes principales en el procesador de fuente de cambios: el contenedor supervisado, el contenedor de concesión, la instancia de proceso y el delegado. el contenedor supervisado tiene los datos a partir de los cuales se genera la fuente de cambios. El componente delegado se puede usar para definir lógica personalizada para procesar aquellos cambios que lea la fuente de cambios. La instancia de proceso hospeda el procesador de fuente de cambios para escuchar los cambios. Se podría representar mediante una máquina virtual, un pod de Kubernetes, una instancia de Azure App Service o una máquina física real. El contenedor de concesión actúa como un almacenamiento de estado y coordina el procesamiento de la fuente de cambios entre varios trabajadores.


1. Va a desarrollar una aplicación.
Deberá establecer las propiedades HTTP estándar de los contenedores en Azure Blob Storage.
¿Qué dos propiedades HTTP se pueden establecer? Cada respuesta correcta presenta parte de la solución.
    - ETag
    - Last-Modified

    Este elemento pone a prueba los conocimientos de los candidatos sobre cómo establecer y recuperar propiedades y metadatos. Los metadatos de los objetos de Azure Storage se definen a través de encabezados a partir de x-ms-meta-. Algunas propiedades HTTP estándar también están disponibles para objetos y contenedores. Las dos únicas propiedades HTTP que están disponibles para contenedores son ETag y Last-Modified.
    Last-Modified, Cache-Control, Origen e Intervalo son propiedades que solo están disponibles para los blobs.


1. Tiene previsto crear una aplicación de Azure Functions basada en scripts de C#.
Es necesario configurar el desencadenador y los enlaces para las funciones de la aplicación de Azure Functions.
¿Qué tiene que hacer?
    - Cree un archivo function.json para cada función.

    Este elemento pone a prueba los conocimientos de los candidatos sobre la configuración de desencadenadores y enlaces.
    Al usar lenguajes de scripting, como el script de C#, el archivo function.json para cada función contendrá sus desencadenadores y enlaces, y deberá crearse explícitamente. El archivo host.json tiene configuraciones específicas del entorno de ejecución, no definiciones de desencadenadores y enlaces. Los métodos y parámetros de decoración se usan para definir desencadenadores y enlaces cuando se usen lenguajes compilados, no los que tengan scripts.


1. Una empresa planea crear una aplicación de Azure Functions.
Necesita recomendar una solución que cumpla los requisitos siguientes:
Ejecutar varias funciones simultáneamente.
Realizar la agregación en los resultados de las funciones.
Evitar los arranques en frío.
Minimizar costes.
¿Qué dos componentes debería recomendar? Cada respuesta correcta presenta parte de la solución
    - El plan Premium
    - Patrón de distribución ramificada de salida y entrada

    Este elemento pone a prueba los conocimientos de los candidatos de Azure Durable Functions y los planes de hospedaje.
    El plan Premium evita los inicios en frío y ofrece una duración de ejecución ilimitada. El patrón de distribución ramificada de entrada y salida permite ejecutar varias funciones en paralelo esperando a que finalicen todas las funciones. A menudo se realiza algún trabajo de agregación en los resultados devueltos de las funciones. El plan Consumo evita pagos por tiempo de inactividad, pero podría encontrarse con arranques en frío. Además, cada ejecución de función se limita a 10 minutos. El patrón de encadenamiento de funciones es una secuencia de funciones que se ejecutan en un orden específico. Con este patrón, la salida de una función se aplica a la entrada de otra función.


1. Está desarrollando una API sin servidor mediante Azure Functions. Se espera que la API gestione un gran número de solicitudes HTTP y realice solicitudes salientes a un servicio de terceros. El servicio de terceros tiene un límite de velocidad en el número de solicitudes que puede gestionar por minuto.
Debe asegurarse de que la función de Azure no supere el límite de velocidad del servicio de terceros y administre el uso de los recursos de forma eficaz.
Cada respuesta correcta presenta parte de la solución. ¿Qué dos acciones debe realizar?
    - Cree la propiedad "maxConcurrentRequests" en el archivo host.json para limitar el número de ejecuciones paralelas.
    - Cree la propiedad "maxOutstandingRequests" en el archivo host.json para limitar el número de solicitudes pendientes en un momento dado.

    Si se usa la propiedad "maxConcurrentRequests" en el archivo host.json, se limitará el número de funciones HTTP que se ejecutan en paralelo, lo que puede ser útil para administrar el uso de recursos y evitar sobrepasar el límite de velocidad del servicio de terceros. La propiedad "maxOutstandingRequests" limita el número de solicitudes pendientes que se mantienen en un momento dado, incluidas las solicitudes en cola y las ejecuciones en curso, que también pueden ayudar a administrar el uso de recursos. No será de utilidad en este caso si se habilita la propiedad "dynamicThrottlesEnabled", ya que rechaza las solicitudes basadas en los medidores de rendimiento del sistema, no en función del límite de velocidad de un servicio de terceros. La propiedad "hsts" se usa para aplicar la función de seguridad de transporte estricta de HTTP (HSTS) y no está relacionada con la administración del uso de recursos o la limitación de velocidad. La propiedad 'routePrefix' sirve para establecer el prefijo de ruta de todas las rutas y no afecta al uso de recursos ni a la limitación de velocidad.


1. Administra una aplicación web de Azure App Service denominada app1. App1 está registrada como aplicación en Microsoft Entra ID.
Deberá asegurarse de que la información de aquellos usuarios con sesión iniciada en Microsoft Entra ID se pueda recuperar mediante app1 a través de Microsoft Graph.
¿Qué debe configurar?
    - permisos delegados

    Este elemento pone a prueba los conocimientos de los candidatos sobre el acceso a los datos de usuario desde Microsoft Graph, que forma parte de la implementación de la autenticación y autorización del usuario.
    Permisos delegados: se utilizan en aplicaciones que tienen un usuario con la sesión iniciada. En estas aplicaciones, el usuario o un administrador da su consentimiento a los permisos que la aplicación solicita y esta puede funcionar como el usuario que ha iniciado sesión al realizar llamadas a Microsoft Graph. appRoles es un atributo en el manifiesto de aplicación de la aplicación registrada que especifica la colección de roles que una aplicación puede declarar. Dichos roles se pueden asignar a usuarios, grupos o entidades de servicio. Los permisos de aplicación los usan las aplicaciones que se ejecutan sin necesidad de que un usuario inicie sesión. Por ejemplo, las aplicaciones que se ejecutan como servicios en segundo plano o los demonios. Un administrador solo puede permitir permisos de aplicación. groupMembershipClaims es un atributo en el manifiesto de aplicación de la aplicación registrada que configura la notificación de grupos emitida en un token de acceso de OAuth 2.0 que espera la aplicación. AppRoles, los permisos de aplicaciones y groupMembershipClaims no permitirán que la información del usuario que haya iniciado sesión se recupere en el código.


1. Tiene previsto generar un token de firma de acceso compartido (SAS) para el acceso de lectura a un blob de una cuenta de almacenamiento.
Debe proteger el token para que no esté en peligro.
¿Qué debe usar?
    - Credenciales de Microsoft Entra ID asignadas al rol Colaborador

    Este elemento pone a prueba los conocimientos de los candidatos sobre las firmas de acceso compartido (SAS) de Azure Storage.
    Las credenciales de Microsoft Entra ID son necesarias para generar el token de SAS. La cuenta usada deberá tener el permiso Microsoft.Storage/storageAccounts/blobServices/generateUserDelegationKey, que está presente en los siguientes roles integrados: Colaborador, Colaborador de la cuenta de almacenamiento, Colaborador de datos de blobs de almacenamiento, Propietario de datos de blobs de almacenamiento, Lector de datos de blobs de almacenamiento y Delegado de blobs de almacenamiento. La clave de cuenta se puede usar para generar el token de SAS, pero podría comprometerse más fácilmente.


1. Una empresa usa Azure API Management para exponer algunos de sus servicios.
Cada desarrollador que consume las API debe usar una sola clave para obtener acceso a varias API sin necesidad de tener la aprobación del publicador de las API.
Debe recomendar una solución.
¿Qué solución debería recomendar?
    - Defina una suscripción con ámbito de producto.

    Este elemento pone a prueba los conocimientos de los candidatos sobre las suscripciones de Azure API Management.
    Al crear un producto, se pueden agregar varias API al producto y se puede asociar una suscripción. No se debe conceder acceso a todas las API. Se debería conceder acceso al desarrollador, independientemente de la dirección IP del autor de la llamada. Un certificado de cliente requeriría una directiva para validar el certificado y lógica específica para asignar al cliente a API específicas.


1. Desarrolla el código siguiente para leer todos los eventos publicados de la primera partición en Azure Event Hubs. (Los números de línea solo se incluyen como referencia).
...
¿Qué dos acciones debe realizar? Cada respuesta correcta presenta parte de la solución.
    - Inserte el siguiente segmento de código en la línea 6: EventPosition startingPosition = EventPosition.Earliest;
    - Inserte el siguiente segmento de código en la línea 7: string partitionId = (await consumer.GetPartitionIdsAsync()).First();

    Este elemento pone a prueba los conocimientos de los candidatos sobre la lectura de eventos de Azure Event Hubs.
    Al insertar el segmento de código que incluye startingPosition = EventPosition.Earliest en la línea 6 se usa la posición inicial más antigua, que es necesaria para leer todos los eventos publicados. Se requiere insertar el segmento de código que incluye string partitionId = (await consumer.GetPartitionIdsAsync()).First(); en la línea 7. El método GetPartitionIdsAsync() devuelve una instancia de string[]. Por lo tanto, el método First() devolverá una cadena. El segmento de código de la línea 6 que usa startingPosition = EventPosition.Latest no usa la posición inicial más antigua. El segmento de código de la línea 7 que incluye int partitionId es incorrecto porque el método GetPartitionIdsAsync() devuelve un string[]. Por lo tanto, el método First() devolverá una cadena y no un valor int, como variable de retorno espera.


1. Es un arquitecto de soluciones en la nube que trabaja para una empresa que tiene previsto exponer su servicio de procesamiento de datos interno, que actualmente se hospeda en Azure, a sus colaboradores a través de las API. El servicio procesa grandes conjuntos de datos e incluye funcionalidades de análisis y generación de informes. La empresa quiere asegurarse de que la API esté bien documentada, que el acceso se controle de forma segura y que se apliquen las directivas de uso.
Debe crear una instancia de Azure API Management que permita el acceso seguro y controlado a las API con la capacidad de aplicar directivas de uso y documentar las API para sus colaboradores.
¿Cuáles tres pasos debe realizar?
    - Configure el acceso a las API mediante la configuración de la autorización de usuario de OAuth 2.0 en la instancia de Azure API Management.
    - Cree una instancia de Azure API Management e importe la API existente mediante la funcionalidad de importación de API de Azure Portal.
    - Implemente directivas para las API en la instancia de Azure API Management para aplicar límites de velocidad y cuotas.

    La opción B es correcta porque al crear una instancia de Azure API Management e importar la API existente, se puede exponer directamente el servicio interno a los colaboradores. La opción C es incorrecta, ya que al configurar una API ficticia, no se cubre la necesidad inmediata de exponer el servicio real. La opción E es correcta porque la implementación de directivas es esencial para controlar el uso de las API. La opción D es incorrecta porque la documentación debe administrarse en la instancia de Azure API Management, no en el código del servicio. La opción A es correcta, ya que la configuración de OAuth 2.0 en la instancia de Azure API Management permite controlar el acceso seguro en las API.


1. Debe leer una fuente de cambios de Azure Cosmos DB mediante un modelo de inserción.
¿Qué dos componentes puede usar para lograr este objetivo? Cada respuesta correcta presenta una solución completa.
    - Azure Functions con un desencadenador de Azure Cosmos DB
    - Biblioteca de procesadores de fuente de cambios

    Este elemento pone a prueba los conocimientos de los candidatos sobre el desarrollo de soluciones que usen el almacenamiento de Azure Cosmos DB.
    Azure Functions con un desencadenador de Azure Cosmos DB permitirá seleccionar el contenedor para conectarse y la función de Azure se desencadenará cada vez que se produzca un cambio en el contenedor. La biblioteca de procesadores de fuente de cambios sigue el patrón de observador, en el que la biblioteca llama a la función de procesamiento. La biblioteca comprueba automáticamente si hay cambios y, si se encuentran, los envía al cliente. Azure Functions con un desencadenador de Event Grid ejecutará la función cuando se envíe un evento de Event Grid. No tiene ninguna relación con una fuente de cambios de Azure Cosmos DB. Un modelo de extracción de fuente de cambios usará un modelo de extracción en lugar de un modelo de inserción.


1. Debe rehidratar un blob almacenado en el nivel de archivo cambiando el nivel de acceso.
¿Qué blob de destino debería usar?
    - Un blob en el nivel de servicio Esporádico de la misma región.

    Este elemento pone a prueba los conocimientos de los candidatos sobre la rehidratación de blobs.
    Los blobs del nivel de archivo solo se pueden rehidratar a los niveles en línea (es decir, Esporádico o Frecuente). El destino puede ser cualquier cuenta de almacenamiento de la misma región.


1. Una empresa usa Azure API Management para exponer sus API a partners externos. La empresa quiere asegurarse de que a las API solo puedan acceder los usuarios autenticados con OAuth 2.0 y que se apliquen cuotas de uso para evitar que se utilice de forma indebida.
Debe configurar la instancia de API Management para cumplir con los requisitos de seguridad y uso.
¿Qué dos acciones debe realizar?
    - Configure una política validate-jwt para autenticar las solicitudes entrantes.

    Es necesario configurar una directiva validate-jwt para autenticar a los usuarios con OAuth 2.0. Para aplicar cuotas de uso, es conveniente configurar un límite de velocidad por directiva principal. El filtrado de IP no cubre los requisitos de autenticación y cuotas. La implementación de una Azure Application Gateway no es necesaria para estos casos específicos.


1. Una empresa está desarrollando una aplicación multiinquilino que controlará grandes volúmenes de eventos de diversos orígenes. La aplicación debe ser capaz de procesar y analizar estos eventos en tiempo real.
Debe diseñar un servicio de ingesta de eventos que permita el aislamiento de los datos y del rendimiento para evitar problemas disruptivos vecinos, al tiempo que tiene en cuenta la complejidad de las operaciones y los costes.
¿Qué modelo de aislamiento de Event Hubs debe implementar?
    - Espacio de nombres dedicado para cada inquilino

    La respuesta correcta es "Espacio de nombres dedicado para cada inquilino", ya que aporta el nivel más alto de aislamiento de datos y rendimiento, algo que es esencial para evitar problemas disruptivos vecinos en una aplicación multiinquilino. "Espacio de nombres compartido con centros de eventos dedicados para cada inquilino" ofrece un aislamiento medio y podría generar problemas disruptivos vecinos. "Espacio de nombres compartido y centros de eventos para todos los inquilinos" aporta el nivel de aislamiento más bajo y no es adecuado para los requisitos indicados. "Multiinquilino de confianza con firmas de acceso compartido" no es un modelo de aislamiento, sino una forma de administrar el acceso dentro de un entorno compartido.


1. Una empresa logística necesita un sistema de mensajería que pueda controlar automáticamente los mensajes que no es posible procesar después de varios intentos y moverlos a un almacenamiento independiente para analizarlos posteriormente.
Debe elegir un servicio de cola que incluya el control automático de mensajes no procesables.
¿Qué debe usar?
    - Cola de mensajes fallidos de Azure Service Bus

    La cola de mensajes fallidos de Azure Service Bus está diseñada para conservar mensajes que no se pueden entregar a ningún receptor o que no se procesan correctamente, lo que viene bien para el requisito de la empresa. Las colas de Azure Storage con tiempo de espera de visibilidad solo ocultan los mensajes temporalmente y no los mueven al almacenamiento independiente. Azure Event Grid con filtrado avanzado está destinado al enrutamiento de eventos, no para controlar el procesamiento de mensajes con errores. Azure Queue Storage con caducidad de mensajes elimina los mensajes vencidos en lugar de moverlos para su análisis.


1. Su empresa está desarrollando una nueva aplicación web que se implementará como una solución en contenedores en Azure. Se prevé que la aplicación tenga cargas de trabajo fluctuantes y que tenga una alta disponibilidad.
Debe crear una solución que permita a la aplicación escalar en función de la demanda y recuperarse frente a errores automáticamente.
Cada respuesta correcta presenta parte de la solución. ¿Qué dos acciones debe realizar? (Elija dos).
    - Implementar la aplicación contenedorizada en Azure Container Apps.
    - Publicar la imagen de la aplicación en Azure Container Registry.

    La publicación de la imagen de la aplicación en Azure Container Registry permite acceder a ella e implementarse fácilmente en los servicios de Azure. Azure Container Apps es un servicio de contenedor sin servidor que escala y se recupera automáticamente frente a errores, lo que hace que sea idea para aplicaciones con cargas de trabajo fluctuantes y requisitos de alta disponibilidad. La ejecución de contenedores mediante Azure Container Instance no permite el escalado automático ni la recuperación frente a errores. Azure Blob Storage no está diseñado para almacenar imágenes de contenedor.


1. Tiene previsto usar una firma de acceso compartido para proteger el acceso a los servicios dentro de una cuenta de almacenamiento de uso general v2.
Es necesario identificar el tipo de servicio que se puede proteger mediante la firma de acceso compartido de delegación de usuarios.
¿Qué servicio debe identificar?
    - Blob

    Este elemento pone a prueba los conocimientos de los candidatos sobre cómo identificar el método de autorización admitido, que es el primer paso de implementarlo.
    El servicio de blobs es el único que admite firmas de acceso compartido de delegación de usuarios. El servicio de archivos admite firmas de acceso compartido de cuenta y servicio. El servicio de cola admite firmas de acceso compartido de cuenta y servicio. El servicio de tablas admite firmas de acceso compartido de cuenta y servicio.


1. Administra una implementación de varias regiones de una cuenta de Azure Cosmos DB denominada account1.
Debe configurar el nivel de coherencia predeterminado para account1. El nivel de coherencia deberá maximizar el rendimiento y minimizar la latencia de las operaciones de escritura.
¿Qué nivel de coherencia debería usar?
    - Posible

    Este elemento pone a prueba los conocimientos de los candidatos sobre la selección del nivel de coherencia, el cual que forma parte del desarrollo de soluciones de Azure Cosmos DB.
    El nivel de coherencia final maximiza el rendimiento y minimiza la latencia. El nivel de coherencia de obsolescencia limitada proporciona un rendimiento menor y una mayor latencia en comparación con las opciones de respuesta restantes. El nivel de coherencia de prefijo coherente proporciona un mayor rendimiento y una latencia más baja para las operaciones de escritura que el nivel de coherencia de sesión, pero un menor rendimiento y una latencia mayor que los niveles de coherencia eventuales. El nivel de coherencia de sesión proporciona un mayor rendimiento y una menor latencia para las operaciones de escritura que el nivel de coherencia de obsolescencia limitada, aunque menor rendimiento y mayor latencia que los niveles de coherencia de prefijo eventuales y coherentes.
